...
Grid.mixin({
	...
	renderBgEvents: function(events) {
		...
		this.renderBgSegsTitle(segs) || segs;
		return this.renderBgSegs(segs) || segs;
	...
	renderBgSegTitleEls:function(segs, disableResizing) {
		var view = this.view;
		var html = '';
		var renderedSegs = [];
		var i;
		...
		if (segs.length) { // don't build an empty html string
			...
			// build a large concatenation of event segment HTML
			for (i = 0; i < segs.length; i++) {
				html += this.bgSegTitleHtml(segs[i], disableResizing);
			}
...
DayGrid.mixin({
	...
	renderBgSegs: function(segs) {
		...
		console.log(allDaySegs);
		return Grid.prototype.renderBgSegs.call(this, allDaySegs); // call the super-method

	},

	renderBgSegsTitle : function(segs) {
		var rowStructs;

		// render an `.el` on each seg
		// returns a subset of the segs. segs that were actually rendered
		segs = this.renderBgSegTitleEls(segs);
	...
	bgSegTitleHtml : function(seg, disableResizing) {
		var view = this.view;
		var event = seg.event;
		var isDraggable = view.isEventDraggable(event);
		var isResizableFromStart = !disableResizing && event.allDay &&
		...
		classes.unshift('fc-day-grid-event', 'fc-h-event','fc-bg-title');
